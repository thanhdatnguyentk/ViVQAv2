...
class IterativeMCANPhrasal(BaseTransformer):
    """
    Iterative MCAN Phrasal Model.
    
    Combines:
    1. Phrasal Encoders (from mcan_phrasal.py logic via config)
    2. Iterative Decoder Architecture (from iterative_mcan.py)
    """
    def __init__(self, config, vocab):
        ...
        # These encoders should be configured as PhrasalEncoder and PhrasalGuidedAttentionEncoder
    ...
    def encoder_forward(self, input_features: Instance):
        ...
        # SA (Phrasal Self Attention via PhrasalEncoder)
        text_features = self.self_encoder(
            features=text_features,
            padding_mask=text_padding_mask
        )

        # GSA (Phrasal Guided Attention via PhrasalGuidedAttentionEncoder)
