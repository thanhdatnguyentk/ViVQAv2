...
class MCANCaptioning(BaseTransformer):
    ...
    def forward(self, input_features: Instance):
        ...
        output = self.decoder(
            answer_tokens=weighted_text_features,
            encoder_features=weighted_vision_features,
            encoder_attention_mask=text_padding_mask
        )
        return F.log_softmax(output, dim=-1)
    def encoder_forward(self, input_features: Instance):
        features = input_features.grid_features

        vision_features, vision_padding_mask = self.vision_encoder(features)

        fused_features = torch.cat([vision_features], dim=1)
        fused_features = self.gelu(self.fusion(fused_features))
        fused_features = self.dropout(fused_features)
        fused_padding_mask = torch.cat([vision_padding_mask], dim=-1)
